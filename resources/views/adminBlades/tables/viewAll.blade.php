    ...
    <tbody id="programTableBody">
        ...
        @foreach($programs as $program)
            ...
            <tr class="programRow" data-division-ids="{{ $divisionIds }}" data-program-id="{{ $program->id }}">
                ...
                <td class="text-center" style= "color: #FFFFFF; background-color: #03592c;" colspan="3">
                    ...
                    <button class="btn btn-sm addActivityInProgramBtn buttonHover" title="Add an activity"
                        ...
                        data-division-name="{{ json_encode($program->divisions->pluck('name')) }}" data-bs-toggle="modal" data-bs-target="#addActivityInProgramModal" style= "background-color: rgb(1, 165, 80);"><i class="fas fa-plus" style= "color: #FFFFFF; -webkit-text-stroke: 1px white;"></i>
                    </button>
            ...
            @foreach($program->projects as $project)
                <tr data-program-id="{{ $program->id }}">
                    <td class="text-left" style= "color: #FFFFFF; background-color:rgb(0, 136, 66);">{{ $project->name }}</td>
                    <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(0, 136, 66);; color: #FFFFFF;">{{ $project->successIndicator }}</td>
                    <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(0, 136, 66);; color: #FFFFFF;">{{ $project->quality }}</td>
                    <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(0, 136, 66);; color: #FFFFFF;">{{ $project->efficiency }}</td>
                    <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(0, 136, 66);; color: #FFFFFF;">{{ $project->timeliness }}</td>
                    <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(0, 136, 66);; color: #FFFFFF;">{{ $project->remarks }}</td>
                    <td class="text-center border border-muted"style="white-space: pre-wrap; background-color:rgb(0, 136, 66);; color: #FFFFFF;">{{ $project->accountable }}</td>
                    <td class="text-center" style= "color: #FFFFFF; background-color:rgb(0, 136, 66);">
                ...
                @foreach($project->subProjects as $subProject)
                    <tr data-program-id="{{ $program->id }}">
                        ...
                        <td class="text-left ps-3" style= "background-color:rgb(1, 165, 80); color:#FFFFFF;">{{ $subProject->name }}</td>
                        <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(1, 165, 80); color: #FFFFFF;">{{ $subProject->successIndicator }}</td>
                        <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(1, 165, 80); color: #FFFFFF;">{{ $subProject->quality }}</td>
                        <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(1, 165, 80); color: #FFFFFF;">{{ $subProject->efficiency }}</td>
                        <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(1, 165, 80); color: #FFFFFF;">{{ $subProject->timeliness }}</td>
                        <td class="text-left border border-muted"style="white-space: pre-wrap; background-color:rgb(1, 165, 80); color: #FFFFFF;">{{ $subProject->remarks }}</td>
                        <td class="text-center border border-muted"style="white-space: pre-wrap; background-color:rgb(1, 165, 80); color: #FFFFFF;">{{ $subProject->accountable }}</td>
                        <td class="text-center" style= "background-color:rgb(1, 165, 80); color: #FFFFFF;" colspan="2">
                            ...
                            <button class="btn btn-sm editSubProjectBtn buttonHover" title="Edit sub-project name" style="background-color: rgb(144, 144, 144);"
                                data-sub-project-id="{{ $subProject->id }}"
                                data-sub-project-name="{{ $subProject->name }}"
                                data-success-indicator="{{ $subProject->successIndicator }}"
                                data-quality="{{ $subProject->quality }}"
                                data-efficiency="{{ $subProject->efficiency }}"
                                data-timeliness="{{ $subProject->timeliness }}"
                                data-remarks="{{ $subProject->remarks }}"
                                data-accountable="{{ $subProject->accountable }}" data-bs-toggle="modal" data-bs-target="#editSubProjectModal"><i class="fas fa-edit" style= "color: #FFFFFF;"></i>
                            </button>
                        ...
                        @foreach($subProject->activities as $activity)
                            <tr data-program-id="{{ $program->id }}">
                                <td class="text-left" hidden>{{ $activity->id }}</td>
                                <td class="text-left ps-4 border border-muted">{{ $activity->name }}</td>
                                ...
                                <td class="text-left border border-muted"style="white-space: pre-wrap;">{{ $activity->accountable }}</td>
                                <td class="text-center border border-muted">
                                    <!-- Add Sub-Activity -->
                                    <button class="btn btn-sm addSubActivityBtn buttonHover" title="Add a sub-activity" style="color: rgb(144, 144, 144); background-color: rgb(212, 212, 212);"
                                        data-activity-id="{{ $activity->id }}" 
                                        data-activity-name="{{ $activity->name }}" data-bs-toggle="modal" data-bs-target="#addSubActivityModal"><i class="fas fa-plus" style= "-webkit-text-stroke: 1px rgb(144, 144, 144);"></i>
                                    </button>
                            ...
                            <!-- Sub-Activities in Activities in Sub-Project -->
                            @foreach($activity->subActivities as $subActivity)
                                <tr data-program-id="{{ $program->id }}">
                                    <td class="text-left" hidden>{{ $subActivity->id }}</td>
                                    <td class="text-left ps-5 border border-muted">{{ $subActivity->name }}</td>
                                    <td class="text-left border border-muted"style="white-space: pre-wrap;">{{ $subActivity->successIndicator }}</td>
                                    <td class="text-left border border-muted"style="white-space: pre-wrap;">{{ $subActivity->quality }}</td>
                                    <td class="text-left border border-muted"style="white-space: pre-wrap;">{{ $subActivity->efficiency }}</td>
                                    <td class="text-left border border-muted"style="white-space: pre-wrap;">{{ $subActivity->timeliness }}</td>
                ...
                @foreach($project->activities as $activity)
                    ...
                    <!-- Sub-Activities in Activities in Project -->
                    @foreach($activity->subActivities as $subActivity)
                        <tr data-program-id="{{ $program->id }}">
                            <td class="text-left" hidden>{{ $subActivity->id }}</td>
                            <td class="text-left ps-3 border border-muted">{{ $subActivity->name }}</td>
